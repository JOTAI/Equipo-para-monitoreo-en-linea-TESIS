...
\chapter{Estructura de la estación}
\label{cap3}
...
\begin{resumen}
  Este capítulo se centra en el proceso de diseño y fabricación que se realizará
  para la estructura de soporte que asegurará la correcta toma de datos e integridad
  de los equipos que serán instalados en una zona sin vigilancia.
\end{resumen}

%-------------------------------------------------------------------
\section{Selección del punto de monitoreo}
%-------------------------------------------------------------------
\label{cap3:sec:seleccionDelPunto}

El rio Chillon es un rio mediano, el cual pasa por \texttt{La Mancomunidad Lima Norte}
y presenta historicamente cada año una alta probabilidad de desborde debido al
fenomeno del niño. Debido a que se desea generar alertas de desborde antes de
que esto pueda ocurrir, ello implica que la estación debe estar instalada antes de los
distritos que pertenecen a la \texttt{La Mancomunidad Lima Norte} por lo que el 
punto de monitoreo tendrá que ubicarse en la provincia de Canta.

El hecho de instalar una estación de monitoreo en linea implicaría que esta se encuentre
sino sobre el rio, muy cerca de él, para poder realizar las mediciones y enviar los datos,
por lo que será necesario contar con un modo de conexión a internet, tomando como la
opción mas simple la conexion a internet por medio de la red celular de algún operador.
Además, ver la posibilidad de apoyarse de alguna infraestructura ya realizada previamente
en el cause del rio como la estructura base de la estación, esto permite reducir el
costo de implementación, ya que no es necesario realizar estudios civiles o hidráulicos
los cuales, además, harían mas compleja la implementación del sistema.

La estructura considerada para base del sistema de monitoreo son los puentes, estos cumplen
la condición de haber sido construidos en base a estudios previos, donde previamente se ha
considerado caudales maximos, condiciones de accesibilidad, materiales de contrucción 
adecuados para la construccion en esa zona, y un diseño estructural suficientemente
resistente para el transporte de personas y vehículos.

El río Chillon cuenta con 4 puentes en el tramo previsto para el proyecto (ver figura~\ref{cap3:fig:mapaPuentesChillon}). El primer puente (-11.725993, -76.966658) se ubica justo en 
el limite fronterizo del distrito de Carabayllo (Es el primer distrito en el cauce del río Chillon
que pertenece a la Mancomunidad de Lima Norte), por lo que detectar un posible desborde y
generar una alerta, deja muy poco tiempo de respuesta para que las autoridades puedan tomar
acción, por lo que este puente queda descartado como estructura para la estación de monitoreo.

El segundo puente (-11.696548, -76.938164) se ubica aproximadamente a 20 minutos en vehículo 
de distancia del primer puente, debido a la distancia es elegible como base estructural de la
estación de monitoreo.

El tercer puente (-11.681010, -76.921471) se ubica aproximadamente a 30 minutos en vehículo de
distancia del primer puente, debido a la distancia y sus condiciones, es elegible como base
estructural de la estación de monitoreo.

El cuarto puente (-11.678369, -76.900007) se ubica aproximadamente a 45 minutos en vehículo de
distancia del primer puente, este puente esta construido para permitir el paso de vehículos y 
personas, sin embargo, su longitud no supera los 10 metros, esto facilitaría el acceso y la
visibilidad de la estación, aumentando su riesgo de robo, por lo que este puente queda descartado
como estructura para la estación de monitoreo. Los demas puentes despues de este último no nos podrían brindar información muy relevante, debido a que a partir de esa ubicación el rio Chillon recibe las
vertientes de diferentes canales de la zona.

Por tanto, quedamos con dos estructuras elegibles como estructura base para la instalación de una
estación de monitoreo, tomando así como elección para el inicio de los diseños al tercer puente,
el \texttt{Puente Macas}.

%(ver figura~\ref{cap3:fig:mapaPuentesChillon})
\figura{Bitmap/03/Mapa_localizacion_puentes_rio_Chillon}{width=0.7\textwidth}%
       {cap3:fig:mapaPuentesChillon}{Mapa de localización de puentes en el río Chillon, vista tomada de maps.google.com}
...
%-------------------------------------------------------------------
\section{Toma de información del punto de monitoreo}
%-------------------------------------------------------------------
\label{cap3:sec:tomaInformacion}

Una vez que se tiene seleccionado el punto donde se realizará el monitoreo,
es necesario recolectar la mayor cantidad de información de la zona y de
la estructura base que se utilizará.

%-------------------------------------------------------------------
\subsection{Conectividad de la zona}
%-------------------------------------------------------------------

Actualmente el Organismo Supervisor de Inversión Privada en Telecomunicaciones - 
\texttt{OSIPTEL} posee un mapa virtual de todas las antenas de comunicación
celular de las empresas del pais, este mapa además, nos permite saber que tipo
de redes son cubiertas (\emph{2G, 3G, 4G, etc.}) en la región que se busca del mapa.

Ingresando a su portal web \emph{http://www2.osiptel.gob.pe/CoberturaMovil/}, nos
dirigimos a la región donde se prevee instalar la estación, y observamos que la
ubicación elegida cuenta con la red 3G, 3.5G y 4G del operador Bitel, y la red
2G, 2.5G del operador Movistar (ver figura~\ref{cap3:fig:coverturaMovil}). Sin embargo,
al ir al lugar se verificó que el operador Movistar solo tiene conexión de telefonía
movil, por otro lado, el operador Bitel si cuenta con conexión 3G en la zona.

%(ver figura~\ref{cap3:fig:coverturaMovil})
\figura{Bitmap/03/CoverturaMovil}{width=0.7\textwidth}%
       {cap3:fig:coverturaMovil}{Mapa de covertura de red de la zona según OSIPTEL.}


%-------------------------------------------------------------------
\subsection{Dimensionado del puente}
%-------------------------------------------------------------------

Al no contar con los planos del puente Macas, se realizó un dimensionado utilizando una wincha
de 3 metros de longitud, de las vigas a las que se tenia acceso, tomando en cuenta que el diseño
estructural del puente es simétrico como se puede observar en la figura~\ref{cap3:fig:basePuenteMacas}.
Se bosqueja los perfiles de las vigas y las dimensiones del puente, considerando esta la mejor
ubicación para la estación debido a que su acceso es complicado y evita que la estación
pueda quedar a la vista.

%(ver figura~\ref{cap3:fig:basePuenteMacas})
\figura{Bitmap/03/BasePuenteMacas}{width=0.7\textwidth}%
       {cap3:fig:basePuenteMacas}{Vista inferior del puente Macas, Canta.}

Se bosqueja las vistas del puente a mano alzada (ver figura )
con los datos recolectados para que puedan ser utilizandos en el diseño de la estructuar
de sujeción al puente de la estacion de monitoreo. 

%-------------------------------------------------------------------
\subsection{Diseño y fabricación de la estructura}
%-------------------------------------------------------------------

En base a los bosquejos realizados del puente Macas se diseña una estructura que permita
sujetar un gabinete metálico de aproximadamente 40X30X20cm el cual contendrá los componentes
principales de la estación. Puesto que el gabinete metalico tendría un peso muy bajo comparado
con la resistencia del acero al corte o tracción, no se considera necesario realizar un 
cálculo y simulación de resistencia estructural, ya que no es el fín de este proyecto ahondar
en la estructura sino en la recolección y transmisión de la información para su posterior análisis.

Se realiza un diseño que soportará el gabinete principal, sujetandolo a una de las vigas
transversales del puente, en el diseño se considera un sistema mecanico que utiliza visagras para
una estructura que pueda ser colocada alrededor de la viga transversal y ser asegurada con un
candado en la base, dificultando con esto la estracción de la base. El gabinete principal se
adosa a una de las caras de la estructura mediante tuercas que se aseguran de manera interna.
Se considera agregar al gabinete principal una cerradora de candado con el fin de aumentar la
dificultad de apertura para la extracción de componentes. Además, se considera que la camará
que se utilizará para la estación debe ubicarse por debajo de esta para poder capturar 
imagenes, ya sea de rio abajo o rio arriba. Obteniendo así una estructura como la que se puede
observar en la figura~\ref{cap3:fig:disenoBaseGabinetePrincipal}.

%(ver figura~\ref{cap3:fig:diseñoBaseGabinetePrincipal})
\figura{Bitmap/03/DisenoBaseGabinetePrincipal}{width=0.7\textwidth}%
       {cap3:fig:disenoBaseGabinetePrincipal}{Plano del diseño de la estructura de sujeción del gabinete principal de la estación de monitoreo.}

Por otro lado, es necesario considerar que tanto la cámara que tomara las fotografías y el sensor
de nivel, necesariamente deben ubicarse en el exterior del gabinete, en diferentes posiciones
con el fin de obtener la información mas pertinente para la generación de alertas y reportes.
Esto implica proteger de hurto ambos componentes, por lo que se elije el diseño de jaula debido
a que el diseño final no depende exactamente de la forma del componente, lo cual brinda facilidad para 
establecer las dimensiones. Con esta consideración se diseña las estructuras basandose en sus dimensiones
 mas importantes como ancho y alto total\footnote{Las caracteristicas técnicas
de ambos componentes se indican en los capítulos siguientes.}, obteniendose así la 
figura~\ref{cap3:fig:disenoProteccionCamara} y la figura~\ref{cap3:fig:disenoProteccionSensorNivel}.

%(ver figura~\ref{cap3:fig:diseñoProteccionCamara})
\figura{Bitmap/03/DisenoProteccionCamara}{width=0.7\textwidth}%
       {cap3:fig:disenoProteccionCamara}{Plano del diseño de la estructura de protección para la cámara D-LINK DCS942l.}

%(ver figura~\ref{cap3:fig:diseñoProteccionSensorNivel})
\figura{Bitmap/03/DisenoProteccionSensorNivel}{width=0.7\textwidth}%
       {cap3:fig:disenoProteccionSensorNivel}{Plano del diseño de la estructura deprotección para el sensor de nivel MB7389.}

Una vez finalizados los planos, estos se llevan a fabricación, el método constructivo es
realizado utilizando doblado de laminas y soldadura. El acabado de los componentes no
requiere de gran presición ni exactitud, por lo que cualquier taller de metal mecánica
puede fabricar los componentes. Obteniendo las estructuras que se observan en la figura~\ref{fig:cap3:estructurasFabricadas}.

\begin{figure}[t]
  \centering
%
  \begin{SubFloat}
    {\label{fig:cap3:estructuraCamara}%
     Estructura de protección de la cámara}%
    \includegraphics[width=0.45\textwidth]%
                    {Imagenes/Bitmap/03/EstructuraProteccionCamara}%
  \end{SubFloat}
\qquad
  \begin{SubFloat}
    {\label{fig:cap3:estructuraSensorNivel}%
     Estructura de protección del sensor de nivel}%
    \includegraphics[width=0.45\textwidth]%
                    {Imagenes/Bitmap/03/EstructuraProteccionSensorNivel}%
  \end{SubFloat}
  % La siguiente no entra; ira en otra 'linea'
  \begin{SubFloat}
    {\label{fig:cap3:estructuraGabinetePrincipal}%
     Estructura de soporte del gabinete principal}%
    \includegraphics[width=0.45\textwidth]%
                    {Imagenes/Bitmap/03/EstructuraBaseGabinetePrincipal}%
  \end{SubFloat}
\caption{Estructuras fabricadas para la estación de monitoreo.%
         \label{fig:cap3:estructurasFabricadas}}
\end{figure}


%% Finalizado el capitulo 3, borrar abajo
